...
namespace ArbolB
{
    internal class ArbolB
    {
        ...
        private Rectangle[] arregloRectangulos;
        ...
        public String Mostrar(ref PictureBox pictureBox, Font font)//metodo  que llama a metodo mostrar esto es para encapsular mas.
        {
            ...
            Rectangle rectangle = new Rectangle(404, 0, 20, 20);
            arbol = Mostrar(padre, 0, null, ref pictureBox, font,x,rectangle);
            dibujarPaginas(ref pictureBox, font, padre, rectangle, Brushes.LightGray);
            return arbol;
        }

        private String Mostrar(Nodo p, int espaciosblancos, string archivo, ref PictureBox pictureBox, Font font, int x, Rectangle rect)// recibe nodo y espacios entre nodos o paginas
        {
            ...
            if (p != null)// si el nodo es diferente de nulo
            {
                ...
                {
                    y += 10;
                    mostrar += Mostrar(p.hijo[i], espaciosblancos + 1, direccion, ref pictureBox, font, xhijo,rect);//muestra las claves de las  páginas hijas
                                                                                                               //si es que el directorio tiene y se manda el nombre
                                                                                                               //del archivo padre donde se encuentran las claves                                                                                             //esto para poder escribirlas en ella por medio de la recursividad
                }
                return mostrar;
            }
            return "";
        ...
        public void dibujarPaginas(ref PictureBox pictureBox, Font font, Nodo p, Rectangle rect, Brush Relleno) 
        {
            ...
            if (p != null)// si el nodo es diferente de nulo
            {
                arregloRectangulos = new Rectangle[p.numclaves];//aqui guardamos posiciones y tamaños de los paginas
                arregloRectangulos[0] = rect;//Se almacena el primer rectángulo 

                int i;

                for (i = 0; i < p.numclaves; i++)// recorre las claves que hay en cada pagina
                {
                    //partiendo del primer rectangulo creado se iran definiendo las psociciones de los demas
                    if (i >= 1)
                    {
                        arregloRectangulos[i] = new Rectangle(arregloRectangulos[i - 1].X + arregloRectangulos[i - 1].Width, arregloRectangulos[i - 1].Y,
                            arregloRectangulos[i - 1].Width, arregloRectangulos[i - 1].Height);
                    }

                    //Se dibuja un rectangulo con relleno
                    g.FillRectangle(Relleno, arregloRectangulos[i]);
                    //Se dibuja el contorno del rectángulo
                    g.DrawRectangle(Pens.Black, arregloRectangulos[i]);

                    //Se dibuja la clave enmedio del rectangulo se posiciona en medio del rectangulo 
                    g.DrawString(p.clave[i + 1].ToString(), font, Brushes.Black,//Dibuja la clave
                    arregloRectangulos[i].X + ((arregloRectangulos[i].Width / 2) - 8), arregloRectangulos[i].Y + ((arregloRectangulos[i].Height / 2) - 11));
                }

                rect.Y += 50;//se aumenta Y en el rectángulo para dibujar más abajo
                ...
                for (i = 0; i <= p.numclaves; i++)//este for recorre las paginas hijas para mostrar las claves
                {
                    if (p.numclaves > 1)
                    {
                        ...
                        {
                            rect.X -= 150;
                            dibujarPaginas(ref pictureBox, font, p.hijo[i], rect, Relleno);//dibuja las claves de las  páginas hijas
                        }
                        else if (i == 1)
                        {
                            rect.X += 150;
